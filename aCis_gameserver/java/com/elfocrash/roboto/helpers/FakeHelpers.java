...
import com.elfocrash.roboto.FakePlayerTitleManager;
...
public class FakeHelpers {
	...
	public static FakePlayer creatSoultaker() {
		int objectId = IdFactory.getInstance().getNextId();
		String accountName = "AutoPilot";
		String playerName = FakePlayerNameManager.INSTANCE.getRandomAvailableName();
		String playerTitle = FakePlayerTitleManager.INSTANCE.getRandomAvailableTitle();

		ClassId classId = getSoultaker().get(Rnd.get(0, getSoultaker().size() - 1));
	...
	public static void giveArmorsByClass(FakePlayer player) {
		...
		case DOMINATOR:
		case DOOMCRYER:
			itemIds = Config.LIST_ARMORS_ROB;
			break;
		case DUELIST:
		case DREADNOUGHT:
		case PHOENIX_KNIGHT:
		...
		case TITAN:
		case MAESTRO:
			itemIds = Config.LIST_ARMORS_HEAVY;
			break;
		case SAGGITARIUS:
		case ADVENTURER:
		case WIND_RIDER:
		...
		case GRAND_KHAVATARI:
			itemIds = Config.LIST_ARMORS_LIGHT;
			break;
		default:
			break;
		}
	...
	public static void giveWeaponsByClass(FakePlayer player, boolean randomlyEnchant) {
		...
		case WIND_RIDER:
		case ADVENTURER:
			itemIds = Config.LIST_WEAPONS_DAGGER;
			break;
		case SAGGITARIUS:
		case MOONLIGHT_SENTINEL:
		case GHOST_SENTINEL:
			itemIds = Config.LIST_WEAPONS_ARCHERS;
			break;
		case PHOENIX_KNIGHT:
		case SWORD_MUSE:
		case HELL_KNIGHT:
		...
		case MAESTRO:
			...
			break;
		case TITAN:
			itemIds = Config.LIST_WEAPONS_TITANS;
			break;
		case DUELIST:
		case SPECTRAL_DANCER:
			itemIds = Config.LIST_WEAPONS_DUELIST;
			break;
		case DREADNOUGHT:
			itemIds = Config.LIST_WEAPONS_DREADNOUGHT;
			break;
		case ARCHMAGE:
		case SOULTAKER:
		case HIEROPHANT:
		...
		case DOMINATOR:
		case DOOMCRYER:
			itemIds = Config.LIST_WEAPONS_MAGES;
			break;
		case GRAND_KHAVATARI:
			itemIds = Config.LIST_WEAPONS_TIRANTY;
			break;
		default:
			break;
		}
		for (int id : itemIds) {
			...
			ItemInstance item = player.getInventory().getItemByItemId(id);
			if(randomlyEnchant)
				item.setEnchantLevel(Rnd.get(Config.FAKE_PLAYERS_ENCHANT_MIN, Config.FAKE_PLAYERS_ENCHANT_MAX));
			player.getInventory().equipItemAndRecord(item);
			player.getInventory().reloadEquippedItems();
		}
	}
	...
	public static List<ClassId> getSoultaker() {
