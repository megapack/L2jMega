...
public class Olympiad
{
	...
	private final class OlympiadAnnouncer implements Runnable
	{
	  ...
	  public void olympiadEnd(Player player)
	  {
	    ...
	    if ((_validationEnd == 0L) || (_period == 0))
	    {
	      player.sendPacket(new CreatureSay(0, Config.ANNOUNCE_ID, "", "Olym comp over: " + numDays + " days, " + numHours + " hours and " + numMins + " minutes :"));
	    }
	    else
	    {
	      player.sendPacket(new CreatureSay(0, Config.ANNOUNCE_ID, "", "Olym comp start: " + numDays + " day(s), " + numHours + " hour(s), " + numMins + " minute(s)."));
	    }
	  }
}