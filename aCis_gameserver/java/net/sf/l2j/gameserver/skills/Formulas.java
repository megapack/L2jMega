...
public final class Formulas
{
	...
	public static double calcBlowDamage(Creature attacker, Creature target, L2Skill skill, byte shld, boolean ss)
	{
	    ...
	    if ((Config.ENABLE_CLASS_DAMAGES) && ((attacker instanceof Player)) && ((target instanceof Player))) {
	        if ((((Player)attacker).isInOlympiadMode()) && (((Player)target).isInOlympiadMode()) && (Config.ENABLE_CLASS_DAMAGES_IN_OLY)) {
	          damage *= OlympiadDamageManager.getDamageMultiplier((Player)attacker, (Player)target);
	        } else {
	          damage *= ClassDamageManager.getDamageMultiplier((Player)attacker, (Player)target);
	        }
	      }
	      if ((Config.ENABLE_CLASS_DAMAGES) && ((attacker instanceof Player)) && ((target instanceof Summon))) {
	        if (((((Player)attacker).isMageClass()) || (((Player)attacker).getClassId().equals(ClassId.ORC_MYSTIC))) && (((Player)attacker).isInOlympiadMode())) {
	          damage /= Config.ALT_MAGE_TO_SUMMON;
	        } else if (((Player)attacker).isInOlympiadMode()) {
	          damage /= Config.ALT_FIGHTER_TO_SUMMON;
	        }
	      }
